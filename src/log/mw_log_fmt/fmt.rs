...
impl<'a> Placeholder<'a> {
    ...
    pub const fn new_debug<T: ScoreDebug>(value: &'a T, spec: FormatSpec) -> Self {
        let value = NonNull::from_ref(value).cast();
        let formatter = |v: NonNull<()>, f: Writer, spec: &FormatSpec| {
            // Borrow checker will ensure that value won't be mutated for as long as the returned `Self` instance is alive.
    ...
    /// Create the placeholder to be represented using `ScoreDisplay`.
    pub const fn new_display<T: ScoreDisplay>(value: &'a T, spec: FormatSpec) -> Self {
        let value = NonNull::from_ref(value).cast();
        let formatter = |v: NonNull<()>, f: Writer, spec: &FormatSpec| {
            // Borrow checker will ensure that value won't be mutated for as long as the returned `Self` instance is alive.
            let typed = unsafe { v.cast::<T>().as_ref() };
...
/// `ScoreDisplay` provides the output in a user-facing context.
/// Replacement for [`core::fmt::Display`].
pub trait ScoreDisplay {
    /// Write display representation of `self` to the provided writer.
    fn fmt(&self, f: Writer, spec: &FormatSpec) -> Result;
}
...
mod tests {
    ...
    use crate::{write, Arguments, FormatSpec, Fragment, Placeholder, ScoreDebug, ScoreDisplay};
    ...
    fn test_write_placeholders_only() {
        ...
        let exp_display_pattern = "true123.4432.2-100-1234-123456-120000000000000000012312341234561200000000000000000test";
        let exp_debug_pattern = "true123.4432.2-100-1234-123456-120000000000000000012312341234561200000000000000000\"test\"";
        assert!(w.get() == format!("{exp_display_pattern}{exp_debug_pattern}"));
    }
