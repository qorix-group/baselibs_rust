...
use crate::{Error, FormatSpec, Result, ScoreDebug, ScoreDisplay, ScoreWrite};
...
pub(crate) fn common_test_display<T: ScoreDisplay + core::fmt::Display>(v: T) {
    ...
    let _ = ScoreDisplay::fmt(&v, &mut w, &FormatSpec::new());
    assert_eq!(w.get(), format!("{v}"));
}

/// Common test comparing [`ScoreDebug`] with [`core::fmt::Display`].
