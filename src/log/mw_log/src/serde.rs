...
use serde::de::{Deserialize, DeserializeSeed, Deserializer, EnumAccess, Error, Unexpected, VariantAccess, Visitor};
        ...
        D: Deserializer<'de>,
    {
        ...
        impl<'de> Visitor<'de> for LevelIdentifier {
            ...
            where
                E: Error,
            {
                let variant = str::from_utf8(value).map_err(|_| Error::invalid_value(Unexpected::Bytes(value), &self))?;
...
mod tests {
    ...
    fn level_token(variant: &'static str) -> Token {
        Token::UnitVariant { name: "Level", variant }
    }

    fn level_bytes_tokens(variant: &'static [u8]) -> [Token; 3] {
        [Token::Enum { name: "Level" }, Token::Bytes(variant), Token::Unit]
    }

    fn level_variant_tokens(variant: u32) -> [Token; 3] {
        [Token::Enum { name: "Level" }, Token::U32(variant), Token::Unit]
    }
    ...
    fn level_filter_bytes_tokens(variant: &'static [u8]) -> [Token; 3] {
        [Token::Enum { name: "LevelFilter" }, Token::Bytes(variant), Token::Unit]
    }

    fn level_filter_variant_tokens(variant: u32) -> [Token; 3] {
        [Token::Enum { name: "LevelFilter" }, Token::U32(variant), Token::Unit]
    }
