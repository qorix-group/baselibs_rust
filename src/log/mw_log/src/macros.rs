...
/// use mw_log_fmt::{Arguments, DisplayHint, FormatSpec, Fragment, Placeholder};
///
/// # let my_logger = mw_log::__private_api::GlobalLogger;
/// let (err_info, port) = ("No connection", 22);
///
/// error!("Error: {} on port {}", err_info, port);
/// error!(target: "app_events", "App Error: {}, Port: {}", err_info, port);
...
macro_rules! __log_enabled {
    ...
    (logger: $logger:expr, target: $target:expr, $lvl:expr) => {{
        let lvl = $lvl;
        lvl <= $crate::STATIC_MAX_LEVEL && lvl <= $crate::max_level() && $crate::__private_api::enabled($logger, lvl, $target)
    }};
}
...
macro_rules! __log_value_serde {
    ($args:expr) => {
        compile_error!("capturing values as `serde::Serialize` requites the `kv_serde` feature of `log`")
    };
}
...
macro_rules! __log_value_error {
    ($args:expr) => {
        compile_error!("capturing values as `std::error::Error` requites the `kv_std` feature of `log`")
    };
}
