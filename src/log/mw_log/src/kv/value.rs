...
pub trait VisitValue<'v> {
    ...
    /// Visit an error.
    #[cfg(feature = "kv_std")]
    fn visit_borrowed_error(&mut self, err: &'v (dyn std::error::Error + 'static)) -> Result<(), Error> {
        self.visit_any(Value::from_dyn_error(err))
    }
}
    ...
    pub fn visit<'v>(inner: &Inner<'v>, visitor: impl VisitValue<'v>) -> Result<(), crate::kv::Error> {
            ...
            V: VisitValue<'v>,
        {
            fn visit_any(&mut self, value: value_bag::ValueBag) -> Result<(), Error> {
                self.0.visit_any(Value { inner: value }).map_err(crate::kv::Error::into_value)
            }
        ...
        inner.visit(&mut InnerVisitValue(visitor)).map_err(crate::kv::Error::from_value)
    }
}
...
pub(in crate::kv) mod inner {
    ...
    */
    ...
    pub fn visit<'v>(inner: &Inner<'v>, mut visitor: impl VisitValue<'v>) -> Result<(), crate::kv::Error> {
        match inner {
...
impl<'v> Value<'v> {
    ...
    #[cfg(feature = "kv_unstable")]
    #[deprecated(note = "downcasting has been removed; log an issue at https://github.com/rust-lang/log/issues if this is something you rely on")]
...
pub(crate) mod tests {
    ...
    fn test_to_value_structured() {
        ...
        assert_eq!("a loong string".to_value().to_token(), inner::Token::Str("a loong string".into()));
    ...
    fn test_to_bool() {
        ...
        for v in unsigned().chain(signed()).chain(float()).chain(str()).chain(char()) {
    ...
    fn test_to_char() {
        ...
        for v in unsigned().chain(signed()).chain(float()).chain(str()).chain(bool()) {
