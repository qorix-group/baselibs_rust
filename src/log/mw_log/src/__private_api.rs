...
pub use std::{module_path, stringify};
...
pub fn log<'a, K, L>(logger: L, args: Arguments<'_>, level: Level, target_module_path_and_loc: &(&str, &'static str, &'static Location), kvs: K)
where
    K: KVs<'a>,
    L: Log,
{
    log_impl(logger, args, level, target_module_path_and_loc, kvs.into_kvs())
}
