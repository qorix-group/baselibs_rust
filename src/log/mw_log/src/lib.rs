...
//!
//! ```
//! // TODO: derive macros not supported yet.
//! // # #[derive(Debug)] pub struct Yak(String);
//! // # impl Yak { fn shave(&mut self, _: u32) {} }
//! // # fn find_a_razor() -> Result<u32, u32> { Ok(1) }
//! // use mw_log::{info, warn};
//! // use mw_log_fmt::{Arguments, DisplayHint, FormatSpec, Fragment, Placeholder};
//! //
...
//!
//! ```
//! // TODO: structured logging not supported.
//! // # use serde::Serialize;
...
//!
//! ```
//! use mw_log::{Record, Level, Metadata};
//!
//! struct SimpleLogger;
//!
//! impl mw_log::Log for SimpleLogger {
...
//!
//! ```
//! # use mw_log::{Level, Metadata};
//! # struct SimpleLogger;
//! # impl mw_log::Log for SimpleLogger {
//! #   fn enabled(&self, _: &Metadata) -> bool { false }
//! #   fn log(&self, _: &mw_log::Record) {}
//! #   fn flush(&self) {}
//! # }
//! # fn main() {}
//! use mw_log::{SetLoggerError, LevelFilter};
//!
//! static LOGGER: SimpleLogger = SimpleLogger;
//!
//! pub fn init() -> Result<(), SetLoggerError> {
//!     mw_log::set_logger(&LOGGER)
//!         .map(|()| mw_log::set_max_level(LevelFilter::Info))
//! }
//! ```
//!
...
//!
//! ```
//! # use mw_log::{Level, LevelFilter, Log, SetLoggerError, Metadata};
//! # struct SimpleLogger;
//! # impl mw_log::Log for SimpleLogger {
//! #   fn enabled(&self, _: &Metadata) -> bool { false }
//! #   fn log(&self, _: &mw_log::Record) {}
...
//! # #[cfg(feature = "std")]
//! pub fn init() -> Result<(), SetLoggerError> {
//!     mw_log::set_boxed_logger(Box::new(SimpleLogger))
//!         .map(|()| mw_log::set_max_level(LevelFilter::Info))
//! }
//! ```
//!
...
///    }
///
///    fn log(&self, record: &mw_log::Record) {
///        if !self.enabled(record.metadata()) {
///            return;
///        }
///
...
///
/// ```
/// use mw_log::{Record, Level, MetadataBuilder};
/// use mw_log_fmt::{Arguments, Fragment};
/// use mw_log_macro::mw_log_format_args;
///
...
impl<'a> RecordBuilder<'a> {
    ...
    pub fn new() -> RecordBuilder<'a> {
        RecordBuilder {
            record: Record {
                args: __private_api::format_args!(""),
                metadata: Metadata::builder().build(),
                module_path: None,
                file: None,
                line: None,
...
/// ```
/// let target = "myApp";
/// use mw_log::{Level, MetadataBuilder};
...
mod tests {
    ...
    fn test_metadata_builder() {
        ...
        let target = "myApp";
        let metadata_test = MetadataBuilder::new().level(Level::Debug).target(target).build();
    ...
    fn test_metadata_convenience_builder() {
        use super::Metadata;
        let target = "myApp";
        let metadata_test = Metadata::builder().level(Level::Debug).target(target).build();
        assert_eq!(metadata_test.level(), Level::Debug);
        assert_eq!(metadata_test.target(), "myApp");
    }
    ...
    fn test_record_builder() {
        ...
        let fmt_args = mw_log_format_args!("hello");
    ...
    fn test_record_key_values_builder() {
        ...
        impl<'kvs> VisitSource<'kvs> for TestVisitSource {
            fn visit_pair(&mut self, _: kv::Key<'kvs>, _: kv::Value<'kvs>) -> Result<(), kv::Error> {
                self.seen_pairs += 1;
                Ok(())
            }
        }
